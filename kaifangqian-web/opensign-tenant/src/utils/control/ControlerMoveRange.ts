...
import {CanvasZoom} from "/@/utils/control/ControlData"

import {bubbleSort,uuid} from "/@/utils"

export function currentMoveRange(opt:any){
  console.log("moveRange",opt);
  
  //计算控件所在页码
  // const page = currentPage(opt);
  const startHeight = CanvasZoom.height * opt.currentPage + CanvasZoom.space * (opt.currentPage+1);
  const endHeight = startHeight + CanvasZoom.height;
  //判断坐标是否移出文档的范围
  if(opt.x < 0){
  	opt.x = 0;
...
export function currentPositionReverse(top: number, pageSize: number) {
    console.log(top, pageSize, top + (pageSize + 1) * CanvasZoom.space + pageSize * CanvasZoom.height, '计算的位置')
    return top + (pageSize + 1) * CanvasZoom.space + pageSize * CanvasZoom.height;
}
