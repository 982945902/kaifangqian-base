 ...
 * @Last Modified time: 2024-05-29 18:39:20
 */
...
enum Api {
    ...
    EnterpriseAuthLog = '/system/tenantInfoExtend/enterprise/auth/log',
    EnterpriseAuthInfo = '/system/tenantInfoExtend/queryById',
    TenantUserJoinCheckFlag = '/sys/sysTenantConfig',
...
export function doLogout() {
    const token = isDevMode() ? (localStorage.getItem('VUE_RESRUN_ADMIN__DEVELOPMENT__2.8.0__COMMON__LOCAL__KEY__') ?
        JSON.parse(localStorage.getItem('VUE_RESRUN_ADMIN__DEVELOPMENT__2.8.0__COMMON__LOCAL__KEY__') as string).value.TOKEN__.value : '') :
        localStorage.getItem('VUE_RESRUN_ADMIN__PRODUCTION__2.8.0__COMMON__LOCAL__KEY__') ?
            JSON.parse(localStorage.getItem('VUE_RESRUN_ADMIN__PRODUCTION__2.8.0__COMMON__LOCAL__KEY__') as string).value.TOKEN__.value : '';
    return defHttp.get({ url: Api.Logout }, { isReturnNativeResponse: true, withToken: false, loginToken: token });
}
export function doLogoutAll() {
    const loginToken = isDevMode() ? (localStorage.getItem('VUE_RESRUN_ADMIN__DEVELOPMENT__2.8.0__COMMON__LOCAL__KEY__') ?
    JSON.parse(localStorage.getItem('VUE_RESRUN_ADMIN__DEVELOPMENT__2.8.0__COMMON__LOCAL__KEY__') as string).value.LOGIN_TOKEN__.value : '') :
    localStorage.getItem('VUE_RESRUN_ADMIN__PRODUCTION__2.8.0__COMMON__LOCAL__KEY__') ?
      JSON.parse(localStorage.getItem('VUE_RESRUN_ADMIN__PRODUCTION__2.8.0__COMMON__LOCAL__KEY__') as string).value.LOGIN_TOKEN__.value : '';
 ...
 * @description: 认证详情
 */
export function getEnterpriseAuthInfo(params) {
    return defHttp.get({ url: Api.EnterpriseAuthInfo, params }, { errorMessageMode: 'none' });
}
/**
 * @description: 认证记录
 */
export function getEnterpriseAuthLog(params) {
    return defHttp.get({ url: Api.EnterpriseAuthLog, params }, { errorMessageMode: 'none' });
}
/**
 * @description: 成员加入申请
 */
export function getJoinMemberData(params) {
    return defHttp.get({ url: Api.TenantUser, params }, { errorMessageMode: 'none' });
}
/**
 * @description: 成员加入申请审核
 */
export function checkTenantUser(params) {
    return defHttp.post({ url: Api.TenantUserCheck, params }, { errorMessageMode: 'none' });
}
/**
 * @description: 成员加入审核开关
 */
export function setJoinFlag(params) {
    return defHttp.post({ url: Api.TenantUserJoinCheckFlag, params }, { errorMessageMode: 'none' });
}
/**
 * @description: 成员加入审核开关
 */
export function getJoinFlag(params) {
    return defHttp.get({ url: Api.TenantUserJoinCheckFlag, params }, { errorMessageMode: 'none' });
}
...
export function checkUserInfoRepeat(params) {
    return defHttp.get({ url: Api.CheckRepeat, params }, { errorMessageMode: 'none',isTransformResponse:false });
}
