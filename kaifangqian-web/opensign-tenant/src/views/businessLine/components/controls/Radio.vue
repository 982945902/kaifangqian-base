...
<template>
    <div :class="['control-'+element.controlType,'control-item','arow-content','this-click',
      ...
      <a-radio-group v-model:value="element.values" style="display:inline" :disabled="element.disabled">
       <template v-for="(item,index) in element.widgetList" :key="index">
         <div class="radio-item-box" :style="[
          ...
          //  'transform: translate('+(item.x - (nowDocument.maxWidth - item.target.width)/2)+'px,'+(item.target.transformHeight + (item.page+1) * 16)+'px);',
           cursorStyle]"
           @mousedown.self="itemMovemousedown($event,item)" >
           <a-radio  :style="radioStyle" :value="item.uid" >
          </a-radio>
         </div>
       </template>
      </a-radio-group>
