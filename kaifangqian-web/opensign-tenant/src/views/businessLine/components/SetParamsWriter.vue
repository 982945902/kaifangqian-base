    ...
    <template v-for="(docItem,docIndex) in documentList">
      <div class="doc-item"  :key="docIndex" v-if="hasWriteControl(docItem.activeControl)">
          <div class="doc-header">
            <SvgIcon name="doc" :size="24"></SvgIcon>
            <a-tooltip>
            ...
            <a-popover title="" trigger="hover" :getPopupContainer="(trigger: Element) => trigger.parentNode" placement="rightBottom">
              <template #content>
                <ul>
                  <li v-for="(signerItem,signerIndex) in options" :value="signerItem.id" :key="signerIndex" @click="handleSetWriterBatch(signerItem,docItem)">{{'全部由'+ signerItem.name + '填写' }}</li>
                </ul>
              </template>
          ...
          <div class="full-parameter-content">
            <ul>
              <li v-for="(item,index) in docItem.activeControl" :key="index" :ref="setLiRefs" class="writer-li">
...
export default defineComponent({
  ...
  setup(props,{emit}) {
    ...
    watch(
      ...
      (newList:[])=>{
        options.value = [];
        let senderList = newList.filter((v:any)=>v.signerType==2 || v.signerType==3);
        senderList.map((m:any,i)=>{
            m.receiveName = '接收方'+ (i+1)
        })
        ...
        console.log(options.value,'下拉选择框')
      },
    )
    onMounted(()=>{
    ...
    function scrollParams(index){
      const li = liRefs.value[index];
      if (li) {
        // rightPanel.value.scrollTop = li.offsetTop;
      }
    }
