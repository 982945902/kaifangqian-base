...
export const checkMiddleText = async (_rule: Rule, value: string) => {
    if (value) {
      if(value.length>=2 && value.length <= 10){
        return Promise.resolve();
      }else{
        return Promise.reject('横排文字的长度2-10位，可以为空');
      }
    }else{
      return Promise.resolve();
    }
};
export const checkBottomText = async (_rule: Rule, value: string) => {
    console.log("checkBottomText",value);
    if (value) {
      if(value.length == 13 || value.length == 14){
        ...
        if(regExp.test(value)){
          ...
          return Promise.reject('下弦文仅支持13或14位的数字');
        }
      }else{
        return Promise.reject('下弦文的长度必须等于13或14位，可以为空');
      }
    }else{
      return Promise.resolve();
    }
