              ...
              <a-col :span="12">
                <a-form-item label="印章审计者" name="auditorList">
                  ...
                  <template v-for="(item, index) in sealFrom.auditorList" :key="item">
                    <a-tag :closable="true" @close="tagHandleClose(3, item)">{{
                      item.authRelationName
                    }}</a-tag>
                  </template>
                  <a-button
                    type="link"
                    @click="selectAdmin(3, '印章审计者')"
                    style="font-size: 12px"
                    >选择审计者</a-button
                  >
                </a-form-item>
  ...
  export default defineComponent({
    ...
    setup(props) {
      ...
      function getSelectUser(item) {
        ...
        if (item.type == 'dept' || item.type == 'depart') {
          return {
