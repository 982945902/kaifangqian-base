...
<template>
  <BasicDrawer
    ...
    <a-card title="基本信息" size="small" v-if="isRoleGroup">
    ...
    <a-card size="small" style="margin-top: 20px;" v-if="!isRoleGroup">
      <template #title>
        纳入权限组
        <span style="font-size:12px;color:#aaa">（选择可管理该角色的权限组）</span>
      </template>
      <div>
        <!-- <a-checkbox-group v-model:value="sysAuthGroup.select" :options="sysAuthGroup.data" /> -->
        <a-checkbox-group v-model:value="sysAuthGroup.checkedList">
          <template v-for="item in sysAuthGroup.data">
            <a-checkbox v-model:checked="item.select"  :value="item.id">{{item.groupName}}
            <a-tooltip placement="top">
                <template #title>
                  <span v-if="item.myFlag">当前用户属于该权限组</span>
                  <span v-else>当前用户不属于该权限组</span>
                </template>
                <Icon icon="ant-design:safety-certificate-filled" :color="item.myFlag?'#127fd2':'#666'"/><br/>
            </a-tooltip>
            <span style="font-size:12px;color:#aaa;line-height: 25px;">{{item.groupDesc}}</span>
            </a-checkbox>
            <br/>
          </template>
        </a-checkbox-group>
    ...
    <a-card size="small"  style="margin-top: 20px;" v-if="isUpdate&&!isRoleGroup">
      <template #title>
        拥有权限组
        <span style="font-size:12px;color:#aaa">（该角色所属的权限组，即该角色下的成员拥有以下权限组的权限）</span>
      </template>
      <!-- <BasicTable @register="registerAuthTable" :rowSelection="{ type: 'checkbox',selectedRowKeys: checkedKeys, onChange: onSelectChange }"> -->
      <BasicTable @register="registerAuthTable">
        <template #toolbar>
...
<script lang="ts">
  ...
  import { Tabs,Card,Select,Modal } from 'ant-design-vue';
  ...
  import { getOrgAuthData,removeOrgAuthData,authGroupListMy} from '/@/api/auth/group';
  ...
  export default defineComponent({
    name: 'RoleDrawer',
    components: { BasicDrawer, BasicForm, Tabs,Select,SelectOption:Select.Option, TabPane: Tabs.TabPane,BasicTable,
    Card,AuthModal,Icon},
    ...
    setup(_, { emit }) {
      ...
      const [registerDrawer, { setDrawerProps, closeDrawer }] = useDrawerInner(async (data) => {
        ...
        //回显权限组
        let sysGropu = await authGroupListMy({});
        console.log("authGroupIds：*----------------------",isUpdate.value,isRoleGroup.value);
        if (unref(isUpdate)) {
          recordId.value = data.record.id;
          setFieldsValue({
            ...data.record,
          });
          let result:any = await getRoleInfo({id:data.record?.id});
          // 回显角色组和角色信息
          ...
          sysAuthGroup.value.checkedList = result.authGroupIds || [];
        }else{
          recordId.value = '';
        }
        ...
        sysAuthGroup.value.data = sysGropu;
      });
      ...
      async function submitTip(){
        var tipFlag = false;
        for(var i=0;i<sysAuthGroup.value.checkedList.length;i++){
          var check = sysAuthGroup.value.checkedList[i];
          var item = sysAuthGroup.value.data.find(e=>{
            return e.myFlag && e.id == check;
          });
          if(item){
            tipFlag = true;
            break;
        ...
        if(!tipFlag){
          Modal.confirm({
            title: '提示',
            content: '该角色未纳入到您所属的权限组中，提交后，您将无法查看和管理该角色，是否确认提交！',
            okText: '提交',
            cancelText: '取消',
            centered:true,
            onOk() {
      ...
      async function handleSubmit() {
        try {
          ...
          // var authGroupIds = [];
          // sysAuthGroup.value.data.forEach(item=>{
          //   if(item.select){
          //     authGroupIds.push(item.id);
          //   }
          // })
          values.authGroupIds = sysAuthGroup.value.checkedList;
          // TODO custom api
          let result = reactive({});
          if(!unref(isUpdate)){  
      ...
      function changeCheckBox(item:any,a:any){
        console.log(item,a)
      }
      return {
        hasPermission,roleGroupSource,checkedKeys,onSelectChange,handleAddAuth,
        handleDeleteAuth,handleSuccess,registerModal, registerDrawer, registerForm,handleTabChange, 
        registerRoleGroupForm, getTitle, handleSubmit ,isUpdate, isRoleGroup, registerTable,
        registerAuthTable,sysAuthGroup,submitTip,changeCheckBox
      };
    },
  });
</script>
