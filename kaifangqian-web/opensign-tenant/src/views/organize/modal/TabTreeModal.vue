         ...
         <div class="source-panel panel-item">
            ...
            <template v-for="item in tabs">
              <TabPane :key="item.type" :tab="item.title"   v-if="!(item.type == 'roleUser' || item.type == 'role') ">
                  <TabTree v-bind="item" @checkChange="checkChange" ref="tabtree" :treeCheckedKeys="treeCheckedKeys"/>
              </TabPane>
              </template>
            </Tabs>
         </div>
...
<script lang='ts'>
  import { defineComponent,ref,computed,unref, onMounted,watch } from 'vue';
  ...
  export default defineComponent({
    ...
    setup(_, { emit }){
      ...
      // const TreeComponent = loadComps('TabTree');
        ...
        watch(
          ...
          (values)=>{
            ...
            console.log(values, '吃热食1111')
            values.map(v=>{
              treeCheckedKeys.value.push(v.id)
            })
          },
          {deep:true}
        )
      ...
      const [registerModal, { setModalProps }] = useModalInner(async (data) => {
        ...
        targetKeys.value = []; //清空缓存
        tabs.value = [];
        console.log("传入：",data);
        if(data.tabs){
          tabs.value = data.tabs;
          console.log(unref(tabs),'传入的tabs')
        ...
        //全部权限和基础权限禁止编辑
        if(data?.groupType == '1' || data?.groupType == '1'){
          setModalProps({ 
            footer:null
          });
        }
      ...
      function handelTabChange(){
        ...
        // tabtree.value&&tabtree?.value[0].setCachedKeys(unref(cacheTargetKey));
      }
