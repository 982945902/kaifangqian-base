...
<template>
  <div class="resrun-tree-table">
      ...
      <template #action>
        ...
        <div :class="prefixCls+'-table'">
            ...
            :rowSelection="{ type: 'checkbox', selectedRowKeys: checkedKeys, onChange: onSelectChange }">
              ...
              <template #toolbar>
                <a-button type="primary" @click="showUserModal" :disabled="isRoleGroup  || !treeData.length" v-if="hasPermission(['role:adduser'])">添加人员 </a-button>
                <a-button type="default" @click="handleDeleteUser" :disabled="isRoleGroup  || !treeData.length" v-if="hasPermission(['role:removeuser'])">移出人员 </a-button>
              </template>
    ...
    <TabTreeModal @register="registerUserModal" @success="handleAddUserSuccess" :tenantAppId="appId"/>
  </div>
</template>
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup(props,{emit}) {
      ...
      async function fetch() {
          let result =  await getRoleTreeList({tenantAppId:props.appId});
          if(result){
      ...
      async function searchRole(val){
        ...
        let roleResult = await getRoleByKeywordList({keyWord:val,pageSize:5,tenantAppId:props.appId});
      ...
      async function handleLoadSearchData(type,page){
        ...
        if(type==='user'){
          let result = await getUserByKeywordList({keyWord:unref(searchText),pageSize:5*page,tenantAppId:props.appId});
        ...
        if(type==='role'){
          let result = await getRoleByKeywordList({keyWord:unref(searchText),pageSize:5*page,tenantAppId:props.appId});
