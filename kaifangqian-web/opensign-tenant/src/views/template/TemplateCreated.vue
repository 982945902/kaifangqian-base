...
<template>
  ...
  <div class="template-content" :style="'height:' + getWindowHeight(63) + 'px'">
    <TemplateHeader :actions="headerActions" :previousStep="previousStep" @cancel="headerCancel" />
      ...
      >
        <a-row>
          <a-col :span="applyFrom.span">
            ...
            >
              <a-input
                v-model:value="applyFrom.templateCode"
                size="middle"
                class="input-width"
                placeholder="请输入模板编号"
              />
          ...
          <a-col :span="applyFrom.span">
            ...
            >
              <a-input
                v-model:value="applyFrom.templateName"
                size="middle"
                class="input-width"
                placeholder="请输入模板名称"
              />
        ...
        <a-row>
          <a-col :span="applyFrom.span">
            ...
            >
              <a-radio-group v-model:value="applyFrom.templateType" @change="templateTypeChange">
          ...
          <a-col :span="applyFrom.span">
            ...
            >
              ...
              >
                ...
                <p class="template-upload-tips"
                  >支持{{ buildFileType(isOffice) }}格式，只能上传1份，文件大小不超过20MB</p
                >
              </a-upload>
            </a-form-item>
          </a-col>
          ...
          <a-col :span="applyFrom.span">
            ...
            >
              <template :key="item" v-for="(item, index) in applyFrom.managerList">
                <a-tag :closable="!formDisabled" @close="tagHandleClose(1, item)">{{
                  item.authRelationName
                }}</a-tag>
              </template>
              <a-button type="link" @click="selectAdmin(1, '模板管理员')" v-if="!formDisabled"
                >选择管理员</a-button
              >
            </a-form-item>
          </a-col>
          ...
          <a-col :span="applyFrom.span">
            ...
            >
              <template :key="item" v-for="(item, index) in applyFrom.userList">
                <a-tag :closable="!formDisabled" @close="tagHandleClose(2, item)">{{
                  item.authRelationName
                }}</a-tag>
              </template>
              <a-button type="link" @click="selectAdmin(2, '模板使用者')" v-if="!formDisabled"
                >选择使用者</a-button
              >
            </a-form-item>
          </a-col>
...
<script lang="ts">
  ...
  // import DocHeader from '../businessLine/components/layouts/DocHeader.vue'
  ...
  export default defineComponent({
    ...
    setup(props) {
      ...
      function docBeforeUpload(file: File, fileList) {
        ...
        const fileTypes = buildFileType(isOffice.value);
        if (!fileTypes.includes(ext)) {
          message.warning(`请上传${fileTypes}格式的文件`);
        ...
        if (!list) {
          ...
          list.forEach((item) => {
            ...
            if (selectUserType.value == 1) {
              const isRepeat = applyFrom.value.managerList.find((item: any) => {
            ...
            if (selectUserType.value == 2) {
              const isRepeat = applyFrom.value.userList.find((item: any) => {
                return item.authRelationId == data.authRelationId;
              });
              if (!isRepeat) {
                // data.authType = 2;
      ...
      function templateTypeChange() {
        if (applyFrom.value.templateType == 1) {
          headerActions.value[0].show = true;
          headerActions.value[1].callBack = setControl;
          headerActions.value[1].text = '下一步';
        } else {
          //saveDataLoading
          headerActions.value[0].show = false;
          headerActions.value[1].callBack = saveDataLoading;
          headerActions.value[1].text = '保存';
        }
        console.log('headerActions:', headerActions.value);
      }
      ...
      function tagHandleClose(type, tag) {
        ...
        if (type == 1) {
          applyFrom.value.managerList = applyFrom.value.managerList.filter(
            (item) => item.authRelationId !== tag.authRelationId,
          );
        } else {
          applyFrom.value.userList = applyFrom.value.userList.filter(
            (item) => item.authRelationId !== tag.authRelationId,
          );
        }
      ...
      function getWindowHeight(offset = 0) {
      ...
      onMounted(() => {
        ...
        console.log('initTemplateAdmin:', router);
        if (query.templateId) {
