...
<template>
    <div>
      <CheckboxGroup v-model:value="element.values" style="display:inline" :name="element.interfaceParamName">
       <template v-for="(item,index) in element.widgetList">
         <div class="radio-item-box" :style="[
           'transform:translate('+item.x+'px,'+item.y+'px)',
           cursorStyle]"
           @mousedown="itemMovemousedown($event,item)" >
...
<script lang="ts" setup>
  ...
  function removeMousedown(){
    ...
    console.log("scoped",scoped,moveInterval.value);
