...
export function getColtrolByType(type:string):any {
	return getControlList(true).filter((item:any) => {
			return item.type == type 
		})[0]
}
export function getControlList(high?:boolean){
  if(high){
    return [...controlList,...highControlList]
  }else{
    return [...controlList,...highControlList];
  }
}

const highControlList:Array<ControlOptins> = [
  {
  	icon:"radio",
  	title:"单选框",
  	name:"单选框",
...
const controlList:Array<ControlOptins> = [
	// {
	// 	icon:"sign",
	// 	name:"个人签名",
	// 	title:"个人签名",
...
export function pageScaling(images:any[]){
  ...
  images.forEach((item:any)=>{
    ...
    maxWidth =parseInt(target.width) > maxWidth?parseInt(target.width):maxWidth;
