...
import { ROLES_KEY, TOKEN_KEY, USER_INFO_KEY, LOGIN_TOKEN_KEY, TENANT_INFO_KEY,APP_WEB_CONFIG} from '/@/enums/cacheEnum';
...
export const useUserStore = defineStore({
  ...
  actions: {
    ...
    async logout(goLogin = false) {
      ...
      const appTenantInfo =  window?.appInfo.tenant_app_info;
      var hash = location.hash || '/#/overview';
      hash = hash.startsWith("/")?hash:"/"+hash;
      const hashPath = hash.split("?");
      const toUrl = `${appAuthInfo.url}/#/auth/login?appId=${appTenantInfo.appId}&appCode=${appTenantInfo.appCode}&redirect=${encodeURIComponent(appTenantInfo.url+hashPath[0])}`;
      console.log("logout：",toUrl);
      //appAuthInfo.url+'/#/auth/login?appId='+ appTenantInfo.appId + '&appCode='+ appTenantInfo.appCode +'&redirect='+ encodeURIComponent(appTenantInfo.url) +  '/' + hash
    ...
    async logoutAll (goLogin = true) {
      ...
      setTimeout(()=>{
        const appAuthInfo =  window?.appInfo.sign_app_info;
        window.open(appAuthInfo.url + '/#/auth/login','_self');
      },1000)
    ...
    async buildWebConfig(){
      if(!this.getWebConfig){
        ...
        const webConfig:Webconfig =  {
          ...
          websiteCopyright: web.website_copyright || "QQ群：482074553  联系电话：15010993257  邮箱：service@resrun.cn  Copyright 2019-2023  北京资源律动科技有限公司  版权所有",
