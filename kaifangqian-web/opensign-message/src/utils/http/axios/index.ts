 ...
 * @Last Modified time: 2023-10-09 16:45:43
 */
...
const transform: AxiosTransform = {
  ...
  requestInterceptors: (config, options) => {
    ...
    const sensitiveInfo= appStore.getSensitiveHeader;
    let {VITE_APP_CODE} = getAppEnvConfig()
    VITE_APP_CODE = VITE_APP_CODE || 'opensign_0001'
    console.log(VITE_APP_CODE,'环境变量code')
    if(sensitiveInfo){
      
      (config as Recordable).headers = Object.assign({
        'resrun-app-code':VITE_APP_CODE
    ...
    if (token && (config as Recordable)?.requestOptions?.withToken !== false) {
      ...
      (config as Recordable).headers['X-Access-Token'] = options.authenticationScheme
        ...
        : token;
        (config as Recordable).headers['resrun-app-code'] = VITE_APP_CODE;
    }
    ...
    deepMerge(
      {
        ...
        // baseURL: globSetting.apiUrl,
         // 基础接口地址
        baseURL: window.eventCenterForAppNameVite? import.meta.env.VITE_BPMS_BASE_API :'/',
