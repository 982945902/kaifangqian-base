 ...
 * @Last Modified time: 2024-04-22 15:17:17
 */
...
enum Api {
  ...
  WebsiteCopyRight = '/sys/websiteCopyright',
  Login = '/sys/login',
  ...
  LoginMobile = '/sys/codeRegisterLogin',
  ...
  JoinedEnterprise = '/system/sysTenantInfo/listMyTenant',
  TenantName = '/system/sysTenantInfo/getTenantNameByKey',
  TenantAuthStatus = '/system/sysTenantInfo/getTenantAuthStatus',
  SignConfig = '/sys/sysUserConfig',
  SignValidateType = '/sys/sysUserConfig/updateType',
  SignValidatePassword = '/sys/sysUserConfig/updatePassword',
}
/**
...
export function checkToken(params, newToken) {
  ...
  return defHttp.get(
    { url: Api.CheckToken, params },
    { isReturnNativeResponse: true, withToken: false, loginToken: newToken },
  );
}
 ...
 * @description: 已加入的企业
 */
export function getJoinedEnterprise(params) {
  return defHttp.get({ url: Api.JoinedEnterprise, params });
}
/**
...
/**
 * @description: 获取系统copyright
 */
export function getWebCopyRight(params): string {
  return defHttp.get({ url: Api.WebsiteCopyRight, params });
}
/**
 ...
 * @description: 获取租户名称
 */
export function getTenantNameByKey(params) {
  return defHttp.get({ url: Api.TenantName, params });
}
...
export function doLogout() {
  const localToken = isDevMode()
    ? localStorage.getItem('VUE_OPENSIGN_ADMIN__DEVELOPMENT__2.8.0__COMMON__LOCAL__KEY__')
      ? JSON.parse(
          localStorage.getItem(
            'VUE_OPENSIGN_ADMIN__DEVELOPMENT__2.8.0__COMMON__LOCAL__KEY__',
          ) as string,
        ).value.TOKEN__.value
      : ''
    : localStorage.getItem('VUE_OPENSIGN_ADMIN__PRODUCTION__2.8.0__COMMON__LOCAL__KEY__')
    ? JSON.parse(
        localStorage.getItem(
          'VUE_OPENSIGN_ADMIN__PRODUCTION__2.8.0__COMMON__LOCAL__KEY__',
        ) as string,
      ).value.TOKEN__.value
    : '';
  return defHttp.get(
    { url: Api.Logout },
    { isReturnNativeResponse: true, withToken: false, loginToken: localToken },
  );
}
 ...
 * @returns
 */
export function getMyTenantDepartsByLoginToken(params?: Object) {
  const loginToken: string = getLoginToken() as string;
  return defHttp.get(
    { url: Api.TenantAppDeparts, params },
    { withToken: false, loginToken: loginToken },
  );
}
/**
 * @desc 开通个人空间
...
export function getTenantAuthStatus(params) {
  return defHttp.get({ url: Api.TenantAuthStatus, params });
}
...
export function getAppTokenByAuthToken(params) {
  ...
  return defHttp.get(
    { url: Api.GetAppTokenByAuthToken, params },
    { withToken: false, loginToken: loginToken },
  );
}
// 判断当前登录token是否有效
...
// 签署配置
export function getSignConfig(params) {
  return defHttp.get({ url: Api.SignConfig, params });
}
// 设置签约顺序
export function setSignValidateType(params) {
  return defHttp.post({ url: Api.SignValidateType, params });
}
// 设置签署密码
export function setSignPassword(params) {
  return defHttp.post({ url: Api.SignValidatePassword, params });
}
