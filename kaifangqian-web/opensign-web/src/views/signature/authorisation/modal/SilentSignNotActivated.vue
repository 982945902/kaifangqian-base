...
<template>
  ...
  <div class="not-activated-container">
    <Loading :loading="compState.loading" :absolute="false" tip="开通中..."></Loading>
    <img src="@/assets/images/activate-service-icon.png" alt="activated Icon" class="status-icon" />
    <h2 class="title">静默签署服务</h2>
    <p class="description-title">
      让签约更便捷
      <p class="description">开通服务并完成认证后，您的授权有效期内，允许在本系统中使用您本人的数字证书完成自动静默签署，无需进行签署意愿校验，静默签署时即代表本人的真实签署意愿 </p>
    </p>
...
import { silentOpenApi,silentQueryRecordApi } from '/@/api/yundun';
...
const activateService = async () => {
  createConfirm({
    ...
    content: '确定开通静默签署授权？',
    async onOk() {
      const asyncPage = window.location.origin + '/#/user/centerInfo?index=5';
