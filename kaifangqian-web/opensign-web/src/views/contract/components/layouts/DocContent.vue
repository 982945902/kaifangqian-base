...
<template>
  <div class="doc-layout-container">
    <slot></slot>
    <div class="doc-content"  @scroll="handleScroll" :style="[
...
export default defineComponent({
  ...
  setup(props,{emit}) {
    ...
    const [registerTable, { setSelectedRowKeys, setTableData }] = useTable({
        ...
        function handlePage(type){
          if(type=='pre'){
            if(currentPage.value==1) return;
              ...
              document.getElementsByClassName('doc-content')[0].scrollTo(0, scrollDis);
          }
          if(type=='next'){
          ...
          return
            ...
            let transformHeight =  document.getElementsByClassName('doc-content')[0].scrollTop / (zoom.value / 100) - currentPage.value * 16;
              props.nowDocument.targets.map((v,i)=>{
                if(transformHeight * (zoom.value / 100) >= v.transformHeight){
        ...
        let throttleInput = throttle(caculateScroll, 100)
...
.doc-layout-container{
  ...
  .doc-content::-webkit-scrollbar-thumb {
    background-color: rgb(190, 188, 188); /* 设置滚动条滑块的颜色 */
  ...
  .doc-content::-webkit-scrollbar-track {
    background-color: #f0f0f0; /* 滚动条轨道的背景色 */
  }
