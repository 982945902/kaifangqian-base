...
export function recaculateControlPosInPage(opt: any,) {
    ...
    console.log(opt, start, end, '计算位置')

    if (top > start && top < end) {
        return;
    }
    if (top > end) {
        opt.position.top = end - opt.size.height + 16
        return;
    }
    if (top < end) {
 ...
 * @return 
 */ 
export function pageScaling(images:any[], allMinWidth?:number){
  var targets:any[] = [];
  var transformHeight = 0;
  var maxWidth = 0;
  const minWidthTarget = images.reduce((min, item) => {
  ...
  images.forEach((item:any)=>{
    ...
    target.allMinWidth = allMinWidth;
    item.target = target;
    targets.push(target);
...
function calculateAspectRatio(original:any){
  ...
  if(original){
    ...
    return{
      width: retainDecimals(newWidth,2),
      height:retainDecimals(newHeight,2),
      ratio: ratio,
...
export function  findMaxImageSize(docs, docKeys, pages) {
  let maxImageWidth = 0;
  ...
  docKeys.forEach(docKey => {
      ...
      if (doc) {
        pages.forEach(pageNum => {
              const image = doc.images.find(img => img.page === pageNum);
              if (image && image.imageWidth > maxImageWidth) {
                maxImageWidth = Number(image.imageWidth);
                maxTarget.maxWidth =  Number(maxImageWidth);
              }
              if (image && image.imageHeight > maxImageHeight) {
                maxImageHeight = Number(image.imageHeight);
                maxTarget.maxHeight = Number(maxImageHeight);
              }                
