...
<template>
  <div>
    <BasicModal v-bind="$attrs" @register="registerModal" :title="getTitle" @ok="handleSubmit" :destroyOnClose="true" :maskClosable="false">
                ...
                <div  class="participants-item-content">
                  <div class="participants-senderUserId-content">
                    ...
                    <div class="org-seal" v-if="item.senderType==4">
                      <p>
                        <span>指定印章：</span>
                        <a-select :disabled="baseVo.signerType==2" style="width:200px" :allowClear="true" @change="(event)=>handleSealSuccess(event,item)" v-model:value='item.senderSealId' :options="sealOptions" :fieldNames="{label:'sealName', value:'sealId'}">  </a-select>
                        <!-- <span>{{ item.senderSealId? item.sealName:'未指定' }}</span>
                        <a-button type="link"  @click="handleSeal(item)">{{ item.senderSealId?'更换':'指定' }}</a-button> -->
                        <!-- <p>{{ item.senderSignType }}</p> -->
                        <!-- <p v-if="checkSealAuth()"> -->
                        <!-- <p v-if="item.senderSignType"> -->
                        <p>
                          <span>盖章方式：</span>
                          <a-checkbox v-model:checked="item.senderSignType" :disabled="baseVo.signerType==2" @change="handleCheckSealAUth(item)">自动盖章</a-checkbox>
                        </p>
                      </p> 
                      <!-- <p>
                        <span>盖章方式：</span>
                      ...
                      <p v-if="item.senderSignType != 1">
                        <span>签署人：
                          <a-tag :closable="baseVo.signerType !=2?true:false"  v-if="item.senderUserId" @close="handleDeletesenderUserId(item)"> {{ item.senderUserName }}</a-tag>
                         </span>
                        <a-button type="link" :disabled="baseVo.signerType==2" v-if="!item.senderUserId" @click="handleAddUser(item)"> <Icon icon="ant-design:plus-outlined"/>添加</a-button>
                      </p>
                      <!-- <p v-if="!item.senderSignType && signConfirm">
                        <span>人脸识别校验：</span>
                        <a-switch :checked-value="1" :un-checked-value="0" :disabled="signerType ==2"  v-model:checked="item.confirmType" />
                      </p> -->
                      <p v-if="!item.senderSignType">
                        <span>签署意愿验证方式：</span>
                        <a-tooltip>
                    ...
                    <div class="org-seal" v-else-if="item.senderType == 2 || item.senderType == 3">
                        <p>
                          ...
                          <a-button type="link"  :disabled="baseVo.signerType==2" v-if="!item.senderUserId.length" @click="handleAddUser(item)"> <Icon icon="ant-design:plus-outlined"/>添加</a-button>
                        </p>
                        <!-- <p v-if="!item.senderSignType && signConfirm">
                          <span>人脸识别校验：</span>
                          <a-switch :checked-value="1" :un-checked-value="0" :disabled="signerType ==2"  v-model:checked="item.confirmType" />
                  ...
                  <div class="signatory-action" v-if="baseVo.signerType !=2">
                    ...
                    <SvgIcon name="down" size="20" v-if="index+1 != senderList.length" @click="handleArrowDown(index)"/>
  ...
  export default defineComponent({
    ...
    setup(_, { emit }){
      ...
      const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
        ...
        baseVo.value = data.record?.baseVo;
      ...
      //指定印章回调
      function handleSealSuccess(val,row){
        currentSealInfo.value.senderSealId = val;
        sealId.value = val
        sealOptions.value.forEach((item:any)=>{
          if(item.sealId == val){
            row.sealName = item.sealName;
          }
        })
        // 更换印章后，自动盖章置为不勾选
        row.senderSignType = false;
          getSealAuthUsers(unref(sealId)).then(res=>{
            if(!checkSealAuth() && row.senderSignType){
              msg.warning('您尚无该印章的使用权限，无法设置自动盖章，请联系企业印章管理员为您授权印章权限');
              row.senderSealId = '';
              sealId.value = '';
              currentSealInfo.value.senderSealId = '';
      ...
      //检查当前操作用户是否有该章使用权限
       function checkSealAuth() {
          ...
          let hasAuth = sealAuthUsers.value.filter(v=>v.tenantUserId == tenantUserId).length?true:false;
          return hasAuth;
      }
      ...
      function  handleArrowDown(index){
        if (index < senderList.value.length - 1) {
          ...
          [senderList.value[index], senderList.value[index + 1]] = [senderList.value[index + 1], senderList.value[index]];
        }
      }
      ...
      function handlePos(){
        ...
        window.open('/#/contract/position?__full__&signReId=' + unref(signReId) + '&signRuId=' + unref(signRuId))
      }
