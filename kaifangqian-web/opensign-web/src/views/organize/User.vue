      ...
      <template #action>
        ...
        <div :class="prefixCls+'-table'">
            <div :class="prefixCls+'-custom-header'">
                <!-- <BasicToolTip  :content="organizeName"  class="wid190"  refName="tooltipOver"></BasicToolTip> -->
                ...
                <div :class="prefixCls+'-custom-btn'">
                    ...
                    <!-- <SensitiveButton type="link" @onClick="handleUserManager(currentDeptId)" sensitiveType="setManager">  
                      <Icon icon="ant-design:user-outlined" />
                      设置主管</SensitiveButton> -->
            ...
            <BasicTable @register="registerTable" @edit-change="onEditChange" :rowSelection="{ type: 'checkbox', selectedRowKeys: checkedKeys, onChange: onSelectChange }">
              ...
              <template #toolbar>
                  ...
                  <a-button type="default" @click="handleResetPassword" v-if="hasPermission(['user:resetpassword'])">密码重置 </a-button>
              </template>
...
<script lang="ts">
  ...
  import { getUserList,getDeptByKeywordList,getUserByKeywordList, getRoleByKeywordList, getDeptTreeList } from '/@/api/demo/system'; 
  import { setBrozenBatch, changeDept, deleteUserBatch, resetPassword } from '/@/api/sys/user';
  import { getMyDeptTreeList,editDept,getDeptManagerList } from '/@/api/sys/dept';
  ...
  export default defineComponent({
    name:'User',
    components: { Button, BasicTree, Row, Col,BasicTable,TableAction, Divider,Icon,DeptDrawer,UserDrawer,TabTreeModal,OrganizeDrawer,RoleDrawer},
    ...
    setup(_,{emit}) {
      ...
      async function handleResetPassword(){
        if(!unref(checkedKeys).length){
          msg.warning('请选择需要重置的用户数据');
          return
        }
        let result = await resetPassword({ids:unref(checkedKeys),password:Md5.hashStr('123456')});
        if(result){
          msg.success('重置成功');
          checkedKeys.value = [];
          reload();
