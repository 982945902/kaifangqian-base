  ...
  <div class="resrun-tree-table">
    ...
    <BasicTable @register="registerTable"   :rowSelection="{ type: 'checkbox', selectedRowKeys: checkedKeys, getCheckboxProps:getCheckboxProps, onChange: onSelectChange }">
      ...
      <template #dataName="{text,record}">
          ...
          <span v-if="record.defaultFlag===1" style="color:#E34D59">(默认)</span>
      </template>
      <template #action="{ record }">
          <a-button type="link" :disabled="record.defaultFlag===1" size="small" @click="onRowClick(record)" >编辑</a-button>
      </template>
    </BasicTable>
...
import {getAuthData, getAllMenuBtnList, deleteAuthData } from '/@/api/auth/group';
