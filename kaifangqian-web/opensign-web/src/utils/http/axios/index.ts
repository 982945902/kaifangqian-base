 ...
 * @Last Modified time: 2024-07-12 16:07:03
 */
...
const transform: AxiosTransform = {
    ...
    transformRequestHook: (res: AxiosResponse<Result>, options: RequestOptions) => {
        ...
        if (hasSuccess) {
            ...
            // console.log("result type:",result,typeof(result));
        ...
        if (code === 530) {
            ...
            createWarningModal({
                ...
                iconType: 'warning',
                onOk() {
                    userStore.setSafeInfo({ phone: result?.phone, username: loginInfo.username,email:result?.email });
    ...
    requestInterceptors: (config, options) => {
        ...
        const signTaskId = ls.get('Sign-Task-Id');
        ...
        if (token && (config as Recordable)?.requestOptions?.withToken !== false) {
            ...
            (config as Recordable).headers['X-Access-Token'] = options.authenticationScheme
                ...
                : token;
            (config as Recordable).headers['resrun-app-code'] = VITE_GLOB_APP_CODE;
            (config as Recordable).headers['sign-task-id'] = signTaskId;
        }
    ...
    responseInterceptorsCatch: (axiosInstance: AxiosInstance, error: any) => {
        ...
        checkStatus(error?.response?.status, msg, errorMessageMode, config);
        ...
        deepMerge(
            {
                ...
                // 基础接口地址
                // baseURL: '/resrun-paas',
                // baseURL: window.eventCenterForAppNameVite? 'http://192.168.0.119:8797/' :'/',
