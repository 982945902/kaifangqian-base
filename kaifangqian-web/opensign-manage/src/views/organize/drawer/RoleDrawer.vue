    ...
    setup(_, { emit }) {
      ...
      const [registerDrawer, { setDrawerProps, closeDrawer }] = useDrawerInner(async (data) => {
        ...
        if (unref(isUpdate)) {
          ...
          let result = await getRoleInfo({id:data.record?.id,tenantAppId:unref(appId)});
          // 回显角色组和角色信息
        ...
        let roleResult = await getRoleGroupList({tenantAppId:unref(appId)});
      ...
      async function handleSubmit() {
        try {
          ...
          let result = reactive({});
          if(!unref(isUpdate)){  
            result = await addRole({...values,tenantAppId:unref(appId)});
          }else{
            result = await updateRole({id:unref(recordId),...values,tenantAppId:unref(appId)});
          }
          if(result){
            msg.success('操作成功');
            closeDrawer();
