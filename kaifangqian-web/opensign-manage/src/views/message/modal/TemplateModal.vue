        ...
        <BasicForm @register="registerForm" >
          ...
          <template #appId="{model,field}">
              <a-select v-model:value="model[field]" >
                <a-select-option v-for="(item,index) in appList" :value="item.id" :key="index">{{ item.appName }}</a-select-option>
              </a-select>
          </template>
        </BasicForm>
...
<script lang='ts'>
  ...
  export default defineComponent({
    ...
    setup(_, { emit }){
      ...
      return {handleButtonConfig, appList, buttonConfigShow, handleParamsAdd,handleParamsDelete,sendTypeList,handleDelete, registerModal,handleAdd, registerTable,createActions,registerForm, getTitle, handleSubmit,templateContent,handleTinymceChange };
    }
  })
</script>
<style lang="less" >
