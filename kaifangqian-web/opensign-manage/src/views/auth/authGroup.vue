...
<template>
  <div class="resrun-tree-table">
    <a-spin :spinning="spinning">
      ...
      >
        <template #action>
            <a-button type="default" @click="handleGroupAdd" :class="[prefixCls+'-action']" v-if="hasPermission(['authcategory:add'])">
    ...
    <div :class="prefixCls+'-table'" style="flex-grow:1">
        <div class="resrun-table-top-header">
            ...
            <div :class="prefixCls+'-custom-btn'">
                  <a href="javascript:;" @click="onEditAuthGroup" v-if="hasPermission(['authcategory:edit'])">
        ...
        <BasicTable @register="registerTable" @edit-change="onEditChange" 
          ...
          <template #toolbar>
            <a-button type="primary" @click="handleAuthAdd"  v-if="hasPermission(['authgroup:add'])">新建 </a-button>
          ...
          <template #groupName="{text,record}">
            ...
            <a-tag v-if="record.groupType ===1 || record.groupType ===2" color="#f50">{{ record.groupType==1?'全部权限':'基础权限' }}</a-tag>
          </template>
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    components: { 
      ...
      Row, 
      Col,
      BasicTable,TableAction, Divider,Icon,GroupDrawer,GroupModal,
      Dropdown,
      DownOutlined,
      Menu,
      MenuItem,
    ...
    setup(props,{}) {
      ...
      async function loadTreeData(){
        ...
        treeData.value = (await getAuthGroup({tenantAppId:unref(appId)})) as unknown as TreeItem[];
