...
<template>
  <div class="sign-contract">
        ...
        >
          <template v-for="(item, index) in nowDocument.images">
            ...
            >
              <img
                style="width: 100%"
                v-lazy="baseUrl + '/file/downloadFileStream/' + item.annexId"
              />
              <div
                class="img-range-line"
                v-if="item.showRange"
                :style="[
          ...
          <draggable
            :list="nowDocument.activeControl"
            ghost-class="ghost"
          ...
          >
            <control-item
              ...
              :isSign="true"
              v-for="(element, index) in nowDocument.activeControl"
              :key="index"
              @controlClick="controlShowMenu"
              @controlMenu="controlMenu"
              @controlMousedown="controlMousedown"
              @refreshControlPosition="refreshControlPosition"
            >
            </control-item>
          </draggable>
        </div>
    ...
    <van-action-sheet v-model:show="batchSetShow" title="批量设置">
      <div class="batch-content">
            ...
            <van-cell-group>
              ...
              >
                ...
                <template #right-icon>
                  <van-checkbox
                    :name="item.id"
                    :ref="el => checkboxRefs[index] = el as any || checkboxRefs[index]"
                    @click.stop
                  />
                </template>
              </van-cell>
        ...
        <div class="batch-config">
          ...
          <span
            v-if="elementControl.propertyPageType == 'custom'"
            style="font-size: 10px; color: #999; margin: 20px 0"
            >（请输入页码或页码范围，以逗号分隔，如1,2,7-10）</span
          >
          <van-field
            v-model="elementControl.custom"
            label=""
            placeholder="请输入页码"
            v-if="elementControl.propertyPageType == 'custom'"
            @blur="checkElementCustom"
          />
    ...
    <div class="sign-document-update" v-show="documentList.length > 1">
      <van-button class="custom-buttons" type="primary" @click="quickHandleDocChange" size="small">
        <SvgIcon
          name="doc-update"
          size="28"
    ...
    <div class="sign-position-location" v-show="hashSignControl">
      <van-button class="custom-buttons" type="primary" @click="locationSignControl" size="small">
        <SvgIcon
          name="sign-location"
          size="28"
          style="text-align: center; display: flex; align-items: center; justify-content: center"
        ...
        <!-- 签署控件数量气泡 -->
        <van-badge
          :content="sealCount + signatureCount + signDateCount + stampSealCount"
          class="count-badge"
        />
      </van-button>
      <div class="doc-footer-operation-tools">
      ...
      <div class="add-control">
        <div class="sign-control">
          <van-space :size="10" v-if="signControlInfo.type == '2'">
            ...
            <span class="custom-link" @touchend="handleAddSeal" v-if="!disabledSignControls"
              >添加</span
            >
          </van-space>
          ...
          <van-space :size="10" v-if="signControlInfo.type == '1'">
            ...
            <span class="custom-link" @touchend="handleAddSignature" v-if="!disabledSignControls"
              >添加</span
            >
          </van-space>
          ...
          <van-space :size="10" v-if="signControlInfo.type == '2' || signControlInfo.type == '1'">
            ...
            <span class="custom-link" @touchend="handleAddSignDate" v-if="!disabledSignControls"
              >添加</span
            >
          </van-space>
          ...
          <van-space :size="10" v-if="signControlInfo.type == '2'">
            ...
            <span class="custom-link" @touchend="handleAddChopSeal" v-if="!disabledSignControls"
              >添加</span
            >
          </van-space>
        </div>
      ...
      <div style="text-align: center">
        <van-space :size="20">
          ...
          <van-button
            ...
            type="primary"
            size="small"
            v-if="!userAuth.status && personalSignAuth == 'required'"
            :disabled="isDetail"
            @click="handleAuth"
            >实名认证</van-button
          >
          <van-button
            ...
            type="primary"
            size="small"
            v-if="userAuth.status || personalSignAuth == 'not_required'"
            :disabled="isDetail"
            @click="handleSign"
            >{{ isSignText ? '签署' : '提交签署' }}</van-button
          >
        </van-space>
      </div>
    </div>
    <van-action-sheet
      v-model:show="signDatevisible"
      :close-on-click-action="true"
      :before-close="beforeCloseSeal"
    >
      <template #default>
    ...
    <van-action-sheet
      v-model:show="sealVisible"
      title="选择企业印章"
      class="seal-set-dialog"
      :showCancelButton="true"
      @confirm="handleSetSeals"
      :beforeClose="beforeCloseSeal"
    >
            ...
            <van-cell style="justify-content: center">
              <div class="seal-row">
                <template v-for="(item, colIndex) in row" :key="item.sealId">
                  <div
                    class="signature-info seal-item"
                    :class="{ 'active-seal-img': signSealId === item.sealId }"
      ...
      <div
        class="seal-modal-action"
        style="position: sticky; bottom: 0; background: #fff; z-index: 10"
      >
        <van-button
          type="default"
          style="margin-left:0;border-left:none;width:100%;hei"
          @touchend="handleSetSeals"
          >确定</van-button
        >
      </div>
    </van-action-sheet>
    <van-action-sheet
      v-model:show="personSignatureVisible"
      title="选择个人签名"
      class="seal-set-dialog"
      :showCancelButton="true"
      @confirm="handleSetSignature"
      :beforeClose="beforeCloseSeal"
    >
      <van-list>
        <div class="seal-list">
          ...
          <!-- 添加横向分割线 -->
          <div
            style="height: 1px; background-color: #e5e5e5; margin: 15px 0"
            v-if="sealType == 'NOLIMIT' || sealType == 'HAND'"
          ></div>
          <div>
      ...
      <div
        :style="{ padding: '3px 0', background: 'var(--van-action-sheet-cancel-padding-color)' }"
      ></div>
      <div
        class="seal-modal-action"
        :style="{ position: 'sticky', bottom: '0', background: '#fff', zIndex: '10' }"
      >
        <van-button
          type="default"
          :style="{ marginLeft: '0', borderLeft: 'none', width: '100%' }"
          @touchend="handleSetSignature"
          >确定</van-button
        >
      </div>
    </van-action-sheet>
    ...
    <v-dialog
      v-model:show="signatureVisible"
      title="签名面板"
      :showCancelButton="true"
      className="signatur-modal"
      @confirm="handleSetSignature"
    ...
    >
      <div class="signature-pad" v-if="showPad">
        ...
        <VueSignaturePad
          ...
          :width="signaturePadConfig.width + 'px'"
          :height="signaturePadConfig.height + 'px'"
          ref="signaturePad"
          :options="options"
        ...
        >
          <template v-for="(char, index) in userFullName.split('')" :key="index">
            ...
            <div
              ...
              :style="{
                width: signatureBlockSize + 'px',
                height: signaturePadConfig.width + 'px',
                fontSize: signatureBlockSize - 30 + 'px',
                margin:
                  firstSignatureBlockTop +
                  'px 0px ' +
                  (signatureBlockSize - signaturePadConfig.width) +
                  'px ' +
                  (signaturePadConfig.width - signatureBlockSize) / 2 +
                  'px',
              }"
            >
              <span>{{ char }}</span>
      ...
      <div class="signature-footer" :style="{ width: signaturePadConfig.height - 40 + 'px' }">
        <div v-if="clientHeight >= 700">
          <span class="signature-tip">
            ...
            >
              <path
                d="M512 64C264.6 64 64 264.6 64 512s200.6 448 448 448 448-200.6 448-448S759.4 64 512 64zm0 820c-205.3 0-372-166.7-372-372s166.7-372 372-372 372 166.7 372 372-166.7 372-372 372zm-40-272h80c4.4 0 8-3.6 8-8v-8c0-4.4-3.6-8-8-8h-80c-4.4 0-8 3.6-8 8v8c0 4.4 3.6 8 8 8zm40-440c-35.3 0-64 28.7-64 64v240c0 35.3 28.7 64 64 64s64-28.7 64-64V236c0-35.3-28.7-64-64-64z"
              ></path>
            </svg>
    ...
    <!-- 人脸识别开通个人账号 -->
    <v-dialog
      v-model:show="needOpenPersonlAccountVisible"
      title="签署提醒"
      :showCancelButton="false"
      :show-confirm-button="false"
    ...
    >
      <div class="certificate-info">
        ...
        <van-button type="default" @click="handleUnSign" :style="{ margin: '0 10px' }"
          >暂不签署</van-button
        >
        <van-button type="primary" @click="faceOpenPersonalAccount" :style="{ margin: '0 10px' }"
          >开通个人账号</van-button
        >
      </div>
    </v-dialog>
    ...
    <v-dialog
      v-model:show="signCertificateVisible"
      title="签署提醒"
      :showCancelButton="false"
      :show-confirm-button="false"
      className="sign-certifcate-modal"
    >
      <div
        ...
        v-if="!certificateInfo.tenantId && certificateInfo.returnCode == '2'"
      >
        <p>您暂未开通个人账号，请开通个人账号</p>
        <van-button type="default" @click="handleUnSign" :style="{ margin: '0 10px' }"
          >暂不签署</van-button
      ...
      <div class="certificate-info" v-if="certificateInfo.returnCode == '1'">
        <p
          >签署文件时，不使用任何数字证书，仅在文件上加盖合成签名和印章图片，<span
            :style="{ color: '#f20404fc' }"
            >无法保护文件也不具备法律效力，</span
          >请知悉！</p
        >
        <van-button type="default" @click="handleUnSign">暂不签署</van-button>
        <van-button type="primary" @click="goSign" :style="{ margin: '0 10px' }"
          >继续签署</van-button
        >
      </div>
      <!-- 测试证书 -->
      <div
        class="certificate-info"
        v-if="certificateInfo.certType == '2' && certificateInfo.returnCode == '2'"
      >
        <p v-if="certificateInfo.holderType == '2'"
          >您尚未拥有可用于电子签约的测试证书，请获取证书</p
        >
        <p v-if="certificateInfo.holderType == '1' && tenantInfo.authStatus != '2'"
          >您尚未拥有可用于电子签约的测试证书，请先实名认证</p
        >
        <p v-if="certificateInfo.holderType == '1' && tenantInfo.authStatus == '2'"
          >您尚未拥有可用于电子签约的测试证书，请获取证书</p
        >
        ...
        <div>
          ...
          <van-button
            ...
            :style="{ margin: '0 10px' }"
            v-if="certificateInfo.holderType == '2'"
            >获取证书</van-button
          >
          <van-button
            type="primary"
            @click="handlePersonAuth"
            :style="{ margin: '0 10px' }"
            v-if="certificateInfo.holderType == '1' && tenantInfo.authStatus != '2'"
            >实名认证</van-button
          >
          <van-button
            type="primary"
      ...
      <div
        class="certificate-info"
        v-if="certificateInfo.certType == '2' && certificateInfo.returnCode == '3'"
      >
        <p v-if="certificateInfo.holderType == '2'"
          >您签署所使用的企业证书已失效，如果您需要继续签署，请申请新的证书</p
        >
        <p v-if="certificateInfo.holderType == '1'"
          >您签署所使用的证书已过期，如果您需要继续签署，请申请新的证书</p
        >
      ...
      >
        <p v-if="certificateInfo.holderType == '2'"
          >您尚未拥有可用于电子签约的数字证书，请联系企业管理员认证企业信息，企业认证通过后，为企业自动下发数字证书</p
        >
        <p v-if="certificateInfo.holderType == '1' && tenantInfo.authStatus != '2'"
          >您尚未拥有可用于电子签约的数字证书，请先实名认证</p
        >
        <p v-if="certificateInfo.holderType == '1' && tenantInfo.authStatus == '2'"
          >您尚未拥有可用于电子签约的数字证书，请获取证书</p
        >
        <p>
      ...
      <div
        class="certificate-info"
        v-if="certificateInfo.certType == '1' && certificateInfo.returnCode == '2'"
      >
        <p
          >您暂未开通个人账号，开通个人账号后，会为您自动颁发平台防篡改证书，用于文件签署时，保护文件不被篡改</p
        >
        <div>
          <van-button type="default" @click="handleUnSign">暂不签署</van-button>
          <van-button
            ...
            :style="{ margin: '0 10px' }"
            v-if="certificateInfo.holderType == '1'"
            >开通个人账号</van-button
          >
        </div>
      </div>
      <div
        class="certificate-info"
        v-if="certificateInfo.certType == '1' && certificateInfo.returnCode == '3'"
      >
      ...
      <div
        class="certificate-info"
        v-if="certificateInfo.certType == '1' && certificateInfo.returnCode == '4'"
      >
        <p
          >您当前用于文件签署的证书是平台下发的防篡改证书，<span :style="{ color: '#f20404fc' }"
            >该证书非CA机构颁发，</span
          >仅用于文件保护，避免文件被篡改，<span :style="{ color: '#f20404fc' }"
            >签署后的文件不具备法律效力，</span
          >请知悉！</p
        >
        <div>
    ...
    <v-dialog
      v-model:show="authCertificateVisible"
      title="签署提醒"
      :showCancelButton="false"
      :show-confirm-button="false"
    ...
    >
      ...
      <p
        >证书有效期：<span :style="{ color: '#f20404fc', marginLeft: '20px' }">{{
          authCerInfo
        }}</span></p
      >
      <van-button
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      showSignControlTip.value =
        localStorage.getItem('showSignControlTip') == 'false' ? false : true;
        ...
        (doc) => {
          if (doc) {
            ...
            if (defaultHasControl.value && controlChangeFlag.value == 'necessary_no_add') {
              ...
              signControlTip.value =
                '请点击签署控件区域的“添加”按钮，设置对应的签署控件，并拖拽至所需的签署位置';
            }
          }
        },
        { deep: true }
      ...
      async function getSignNodeInfo() {
        ...
        if (userCheckResult.holderType === TenantTypeEnum.PERSONAL) {
          ...
          if (platePersonSignAuth === 'required' || platePersonSignAuth === 'not_required') {
            ...
            if (
              (ruInfo && ruInfo.baseVo.personalSignAuth === 'required') ||
              ruInfo.baseVo.personalSignAuth === 'not_required'
            ) {
      ...
      async function checkStatus() {
        if (
          tenantInfo.tenantType == 1 &&
          (tenantInfo.authStatus != 2 || userInfo.personalTenant.authStatus != 2)
        ) {
          router.replace({
            path: '/check/auth',
            query: {
              ...router.currentRoute.value.query,
              redirect: encodeURIComponent('/signContract'),
            },
          });
        }
        ...
        signControl = {
          ...
          size: {
            width: newSignatureSize.value.width
              ? newSignatureSize.value.width
              : signControl.size.width,
            height: newSignatureSize.value.height
              ? newSignatureSize.value.height
              : signControl.size.height,
          },
      ...
      function reCaculatImgSize(orisignSize) {
        ...
        if (
          newSignatureSize.value.width / newSignatureSize.value.height ==
          orisignSize.width / orisignSize.height
        ) {
          return orisignSize;
        }
        if (
          ...
          return {
            width: newSignatureSize.value.width,
            height: parseInt(
              orisignSize.height / (orisignSize.width / newSignatureSize.value.width)
            ),
          };
        }
        if (
          ...
          return {
            width: parseInt(
              orisignSize.width / (orisignSize.height / newSignatureSize.value.height)
            ),
            height: newSignatureSize.value.height,
          };
        }
      ...
      //更新所有签名图片
      function updateAllSignatureImg(
        controls: any,
        sealId: string,
        annexId: string,
        sealSize?: any
            ...
            if (stroke && stroke.points && Array.isArray(stroke.points)) {
              for (let i = 1; i < stroke.points.length; i++) {
                ...
                if (
                  prevPoint &&
                  currPoint &&
                  typeof prevPoint.x === 'number' &&
                  typeof prevPoint.y === 'number' &&
                  typeof currPoint.x === 'number' &&
                  typeof currPoint.y === 'number'
                ) {
                ...
                h(
                  ...
                  [
                    ...
                    h(
                      ...
                      [
                        h('strong', '重要声明：'),
                        '平台防篡改证书仅能确保文件在签名后不被篡改，不具备《电子签名法》规定的法律效力。',
                      ]
                    ),
                    h(
                      'p',
                      ...
                      [
                        '点击“',
                        h('strong', '同意，继续签署'),
                        '”即代表您已知悉非实名认证签署的相关风险，并同意以该方式完成本次签署。',
                      ]
                    ),
                  ]
                ),
      ...
      function signPosition(signerType: string) {
        // 定义控件优先级顺序
        const controlPriority = signerType === 'person' ? ['signature'] : ['seal', 'chop-stamp'];

        // 在当前文档中查找控件
        const currentDocControls = nowDocument.value.activeControl || [];
        for (const controlType of controlPriority) {
          const targetControl = currentDocControls.find(
            (item: any) => item.controlType === controlType
          );
          if (targetControl) {
              ...
              h(
                ...
                [
                  ...
                  h(
                    ...
                    [h('div', '1. 点击【立即认证】'), h('div', '2. 在「云盾系统」完成个人实名认证')]
                  ),
                ]
              ),
...
<style lang="less" scoped>
  ...
  .seal-list {
    ...
    .seal-selected-icon {
      ...
      width: 20px;
      height: 20px;
      background: url('data:image/svg+xml;utf8,<svg fill="none" viewBox="0 0 24 24" stroke="green" xmlns="http://www.w3.org/2000/svg"><circle cx="12" cy="12" r="11" stroke="%23288cf2" stroke-width="2" fill="white"/><path d="M7 13l3 3 7-7" stroke="%23288cf2" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>')
