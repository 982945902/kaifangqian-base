...
<template>
    <div :class="['control-'+element.controlType,'control-item','arow-content','this-click',
      ...
      <van-checkbox-group v-model="element.values" class="control-checkbox" :disabled="element.disabled">
        <template  v-for="(radionItem,radionIndex) in element.widgetList" :key="radionIndex" >
            <div class="radio-item-box" :style="[
            'transform:translate('+ (radionItem.x  - element.position.left) +'px,'+(radionItem.y - element.position.top) +'px)',
            cursorStyle,
            'max-width:32px',
            'max-height:32px',
            ]">

                <van-checkbox :name="radionItem.uid" shape="square" icon-size="18px"></van-checkbox>
            </div>
        </template>
        </van-checkbox-group>
    </div>
