...
<template>
  ...
  element.controlClick?'click':'default']">
    <van-radio-group v-model="element.value"  class="control-radio" :disabled="element.disabled">
      <template v-for="(item,index) in element.widgetList" :key="index">
        <div class="radio-item-box" :style="[
         'transform:translate('+ parseInt((item.x   - element.position.left) +'')  +'px,'+parseInt((item.y - element.position.top)+'') +'px)',
         'max-width:32px',
         'max-height:32px'
         ]">
        <van-radio  :name="item.uid"></van-radio>
      </div>
      </template>
    </van-radio-group>
...
import {moveRange,currentPosition,currentPositionReverse,getMoveScoped} from "../data/ControlerMoveRange"
...
console.log(props.element.properties,'传进来的控件----')

const baseImgWidth = window.innerWidth - 20;

const aspecRatio = baseImgWidth / 800;

const radioSize = 32 * aspecRatio;
...
:deep(.van-radio__icon .van-icon){
  width: calc(var(--aspecRatio) * 60px);
  height: calc(var(--aspecRatio) * 60px);
  max-width: calc(var(--aspecRatio) * 30px);
  max-height: calc(var(--aspecRatio) * 30px);
}
...
.radio-item-box{
  width: calc(var(--aspecRatio) * 64px);
  height: calc(var(--aspecRatio) * 64px);
  display: flex;
  align-items: center;
